...
from pyspark.sql.types import StructField, StructType, IntegerType, StringType, MapType, BooleanType, ArrayType
...
class TransformData():
    ...
    def create_spark_df(self):
        return self.spark_context.createDataFrame(data= self.json_data, schema= schema)
    
    def filter_created_at(self):
        df = self.create_spark_df()
        df = df.filter(df[self.date_col].between(self.date_start, self.date_end))
        return df
    
    def build_schema(self, df, df_column):
        json_schema = self.spark_context.read.json(df.rdd.map(lambda row: row[df_column])).schema
        return json_schema
